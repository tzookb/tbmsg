...
class EloquentTBMsgRepository implements iTBMsgRepository
{
    ...
    public function getNumOfUnreadMsgs($user_id) {
        $results = $this->db->select(
            '
            SELECT COUNT(mst.id) as num_of_unread
    ...
    public function getConversationMessages($conv_id, $user_id, $newToOld = true)
    {
        ...
        return $this->db->select(
            '
            SELECT msg.id as msg_id, msg.content, mst.status, msg.created_at, msg.sender_id as user_id
            FROM '.$this->tablePrefix.'messages_status mst
    ...
    public function getConversations($user_id)
    {
        return $this->db->select(
            '
            SELECT msg.conv_id as conv_id, msg.created_at, msg.id msg_id, msg.content, mst.status, mst.self, us.'.$this->usersTableKey.' user_id
